...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    do
    {
        ...
        std::string _filenameAscii = WideToASCII(_filename);
...
DialogResult Eff2EmtConverter::ConvertZone(const wchar_t* EQFolder, wchar_t* ZoneNick, std::string &ZoneNickAscii)
{
    ...
    size_t _emtFileSize = FileSize(_zoneSoundEmitterFilename.c_str());
    ...
    while (!_effFile.eof())
    {
        ...
        _sound1.FadeInMS = Math_Min(_sound1.FadeOutMS / 2, 5000); // Cap at 5 second FadeIn. Some music entries have a looooong FadeOut.
        ...
        if (_effEntry.SoundType != 1) // Music files ignore cooldowns in ZoneNick.emt.
        {
            ...
            _sound1.MinRepeatDelay = (_sound1.WavLoopType == 0) ? 0 : Math_Max(_effEntry.Cooldown1, 0);
            _sound1.MaxRepeatDelay = (_sound1.WavLoopType == 0) ? 0 : Math_Max(_effEntry.Cooldown1, 0) + Math_Max(_effEntry.RandomDelay, 0);
        }
        _sound1.WavFullVolRadius = (_effEntry.SoundType == 0) ? _effEntry.Radius : Math_Max(_effEntry.FullVolRange, 0);
        ...
        else
        {
            ...
            if (_effEntry.SoundType != 1) // Music files ignore cooldowns in ZoneNick.emt.
            {
                ...
                _sound2.MinRepeatDelay = Math_Max(_effEntry.Cooldown2, 0);
                _sound2.MaxRepeatDelay = Math_Max(_effEntry.Cooldown2, 0) + Math_Max(_effEntry.RandomDelay, 0);
            }
