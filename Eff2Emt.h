...
Int32 Math_Min(Int32 value1, Int32 value2) { return (value2 < value1) ? value2 : value1; }
Int32 Math_Max(Int32 value1, Int32 value2) { return (value2 > value1) ? value2 : value1; }

bool FileExists(const wchar_t* Path) { struct _stat buffer; return (_wstat(Path, &buffer) == 0); }
size_t FileSize(const wchar_t* Path) { struct _stat buffer; return (_wstat(Path, &buffer) == 0 ? buffer.st_size : 0); }

std::string WideToASCII(wchar_t* Text)
{
    int _len = wcslen(Text);
    char _text[256];
    memset(_text, 0, sizeof(_text));
    wcstombs(_text, Text, Math_Min(_len, 255));
